...
- [ ] 如提交修改已发布 iteration，已确认没有将原计划基线覆盖为不同目标的执行记录。
- [ ] 如本提交承载实施结果，已按 [任务收口与完成声明](TASK-CLOSURE.md) 核对结论；
      `Partial` / `Blocked` 不伪装成完成提交说明。
...
6. 修改已发布的 planned iteration 前，先对照其首次提交版本；改线必须追加说明并新建编号。
7. 提交不能替代收口：先记录验证、状态同步和残余归口，再将真实完成边界写入提交说明或最终回复。
...
实施结果提交前还必须给出 `TASK-CLOSURE.md` 定义的完成声明；如果本次提交只是
`Partial` 的阶段产物，说明仍未完成的归口，而不是暗示故事已经 `Done`。
