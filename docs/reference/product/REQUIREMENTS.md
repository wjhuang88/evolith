...
> **注意**：本文档基于旧定位（AI Agent Harness 平台）编写。2026-06-23 战略转型后，产品方向调整为 AI 原生 Git 平台。完整新方向见 [GIT-CENTRIC-PLATFORM 提案](../../proposals/GIT-CENTRIC-PLATFORM.md)。下方内容保留作为历史参考，不代表当前产品路线。

## 1. 项目概述

### 1.1 项目背景

随着 AI Agent 技术的快速发展，开发者需要一个既支持传统 Git 工作流、又能让 AI 智能体直接参与编码和交付的平台。现有方案存在以下问题：
- 传统 Git 平台（GitHub/GitLab）对 AI Agent 协作缺乏原生支持
- AI 编程工具（Cursor/Copilot）与代码托管割裂，需要复制粘贴
- Agent 技能（Skill/MCP/CLI）缺乏统一的发现和分发机制

### 1.2 项目目标

构建一个 AI 原生的 Git 平台，提供：
1. 标准 Git 仓库托管，内置 AI Agent 协作能力
2. Vibe Coding——AI 智能体直接在仓库中编码、提交和审查
3. Agent 技能中心——从仓库自动索引 Skill / MCP 工具 / CLI 接口
...
> 变更说明：原“代码片段仓库”已经由 [ADR-0002](../../decisions/ADR-0002-cli-friendly-interface-replaces-snippet.md)
> 调整为 CLI 友好接口仓库。下方原片段功能保留为迁移参考，不再作为新功能实现目标。

#### 2.3.1 功能描述

提供面向大模型优化的 CLI 接口描述管理，支持分类、搜索、直接引用和本地 CLI 复用。
...
提供应用内内部文档浏览能力。文档内容以 Markdown 文件维护在独立目录中，前端文档页面根据该目录生成文档列表并渲染文档详情。

#### 2.4A.2 功能列表

| 功能ID | 功能名称 | 优先级 | 描述 |
|--------|----------|--------|------|
| F1.4A.1 | 独立文档源目录 | P1 | 内部文档 Markdown 与项目治理 `docs/` 分离维护 |
| F1.4A.2 | 文档列表 | P1 | 文档页面展示可浏览的 Markdown 文档列表 |
| F1.4A.3 | Markdown 渲染 | P1 | 支持标题、段落、列表、代码块等基础 Markdown 展示 |
| F1.4A.4 | 空状态 | P2 | 文档目录为空时页面展示可理解的空状态 |

#### 2.4A.3 约束

- 初始版本不提供在线编辑、全文搜索、权限分级或版本历史。
- 初始版本不复用项目治理 `docs/` 目录作为应用内文档源。
- 默认优先采用前端静态或构建时加载，除非实现时证明需要后端 API。
