...
| EVO-115 | git Smart HTTP WWW-Authenticate + 真实 git-client E2E | Proposed | P1 | EVO-103-B-2 收口残余；git 客户端等 401 challenge → 需 WWW-Authenticate: Basic on /repos/ 401；解禁真实 clone/push/pull E2E | [Item file](active/EVO-115-git-www-authenticate-and-real-e2e.md) |
...
| EVO-103-B-2 | Smart HTTP 端点实现（git subprocess 流式处理 3 个端点） | Review | P0 | EVO-103-B 子 Story；硬依赖 EVO-103-B-1（Done）；依赖 EVO-103-A（Done）；ITERATION-046 | [Item file](active/EVO-103-B-2-smart-http-endpoints.md)<br>[ADR-0004](../decisions/ADR-0004-git-centric-storage.md)<br>[EVO-103-B-1](active/EVO-103-B-1-git-client-auth-infra.md) |
