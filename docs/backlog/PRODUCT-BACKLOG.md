...
> **2026-06-23 方向调整**：主线从 "Skill/CLI/MCP DB Registry" 转向 **Git 托管 + Vibe Coding 平台 + Pages 式 skill/CLI/MCP 索引**。详见 [Git-Centric Platform Proposal](../proposals/GIT-CENTRIC-PLATFORM.md) + [ADR-0004](../decisions/ADR-0004-git-centric-storage.md) + [ADR-0005](../decisions/ADR-0005-deprecate-sandbox-runtime.md)。新方向 Epic EVO-100 已进入 Phase E' 实施；EVO-101 / EVO-102 已由 Iteration 042 完成，EVO-103 A/B/C 与 EVO-116 验收硬化已完成，当前建议优先进入 EVO-112 仓库管理 UI，再推进 EVO-105/EVO-106/EVO-107 agent 集成链路。EVO-104 的 UX gate（U-01~U-05）已解除；EVO-104 实现仍等待 API / 基础 UI 依赖。旧 skill/CLI/MCP lifecycle 相关项（EVO-019/020/027/028/029/045/046/047/049/049-B/050）按 Superseded/Dropped 标注。

## Current Priorities

| ID | Title | Status | Priority | Decision Context | Required Reads |
| --- | --- | --- | --- | --- | --- |
| EVO-112 | Repo Management UI（仓库列表 / 创建 / 详情 / 导航重构） | Proposed | P0 | EVO-103/EVO-116 已关闭；下一步需要把 Git 仓库能力暴露为用户可操作的主入口，建议拆成列表/创建/导航基础与详情浏览两轮 | [Item file](active/EVO-112-repo-management-ui.md)<br>[Two-month plan](../roadmap/TWO-MONTH-PLAN-2026-07.md)<br>[Design System](../reference/DESIGN.md) |
| EVO-105 | Commit API + 直推直合 + Promote API | Proposed | P0 | EVO-112 后进入 Phase E'-2；写路径、policy evaluator、audit 是 agent 集成和 vibe coding 的后端核心 | [Item file](active/EVO-105-commit-and-promote-api.md)<br>[EVO-100](active/EVO-100-git-centric-platform-foundation.md)<br>[Git-Centric proposal](../proposals/GIT-CENTRIC-PLATFORM.md) |
| EVO-106 | Agent Session API + Scoped Token | Proposed | P0 | 与 EVO-105 共同形成外部 agent engine integration surface；scope 模型必须复用 EVO-116 权限边界 | [Item file](active/EVO-106-agent-session-and-scoped-token.md)<br>[EVO-100](active/EVO-100-git-centric-platform-foundation.md)<br>[Git-Centric proposal](../proposals/GIT-CENTRIC-PLATFORM.md) |
| EVO-116 | EVO-103 验收硬化：API key scope、Context API 边界、元数据同步与合约补齐 | Done | P0 | 2026-06-26 架构组对 EVO-103 Conditional Accept；2026-06-26 全部 5 个验收面（API key scope、Context API 边界、push metadata、API contract、性能证据）实现完成，ITERATION-049 闭环 Complete | [Item file](active/EVO-116-evo-103-acceptance-hardening.md)<br>[EVO-103 acceptance](../review/EVO-103-acceptance.md)<br>[EVO-103 architecture review](../review/EVO-103-architecture-review.md)<br>[ADR-0006](../decisions/ADR-0006-smart-http-via-git-subprocess.md) |
...
| EVO-115 | git Smart HTTP WWW-Authenticate + 真实 git-client E2E | Done | P1 | EVO-103-B-2 收口残余；git 客户端等 401 challenge → 需 WWW-Authenticate: Basic on /repos/ 401；解禁真实 clone/push/pull E2E（ITERATION-047） | [Item file](active/EVO-115-git-www-authenticate-and-real-e2e.md) |
| EVO-116 | EVO-103 验收硬化：API key scope、Context API 边界、元数据同步与合约补齐 | Done | P0 | EVO-103 Conditional Accept 发布前阻断项；ITERATION-049 已闭环 Complete（2026-06-26）；EVO-103 现可由架构组批准正式 Accept | [Item file](active/EVO-116-evo-103-acceptance-hardening.md)<br>[EVO-103 acceptance](../review/EVO-103-acceptance.md)<br>[EVO-103 architecture review](../review/EVO-103-architecture-review.md)<br>[ADR-0006](../decisions/ADR-0006-smart-http-via-git-subprocess.md) |
| EVO-117 | 当前架构与依赖文档基线校准 | Done | P1 | 2026-07-29 仓库审查；修复 Rust 版本、模块化单体、Git-centric 主线、依赖版本和 legacy Sandbox 文档漂移 | [Item file](active/EVO-117-current-documentation-baseline.md) |
| EVO-100 | Epic: Git-Centric Platform Foundation | In Progress | P0 | 用户反馈 2026-06-23 方向调整; Epic 容器；EVO-101/102/103/113/116 已 Done；下一步推进 Repo UI / agent / indexer 主线 | [Item file](active/EVO-100-git-centric-platform-foundation.md)<br>[Proposal](../proposals/GIT-CENTRIC-PLATFORM.md)<br>[ADR-0004](../decisions/ADR-0004-git-centric-storage.md)<br>[ADR-0005](../decisions/ADR-0005-deprecate-sandbox-runtime.md) |
...
| EVO-103 | Repo CRUD + Smart HTTP + Repo Context API（父项，拆为 A/B/C） | Done | P0 | Epic EVO-100 子 Story；现为 A/B/C 父项，详见子 Story；依赖 EVO-101 | [Item file](active/EVO-103-repo-context-and-smart-http.md) |
...
| EVO-103-B | Smart HTTP git 协议（父项，拆为 B-1/B-2） | Done | P0 | EVO-103 子 Story；现为 B-1/B-2 父项，详见子 Story；硬依赖 EVO-103-A；B-1+B-2 全部 Done，父项完成 | [Item file](active/EVO-103-B-smart-http-git-protocol.md)<br>[ADR-0004](../decisions/ADR-0004-git-centric-storage.md)<br>[EVO-103-A](active/EVO-103-A-repo-crud.md) |
| EVO-103-B-1 | Git 客户端认证基础设施（Basic-Auth + RBAC 端点挂载 + CSRF 豁免） | Done | P0 | EVO-103-B 子 Story；依赖 EVO-103-A（Done）；现有 ApiKeyRepository（ITERATION-045） | [Item file](active/EVO-103-B-1-git-client-auth-infra.md)<br>[ADR-0004](../decisions/ADR-0004-git-centric-storage.md)<br>[EVO-103-A](active/EVO-103-A-repo-crud.md) |
| EVO-103-B-2 | Smart HTTP 端点实现（git subprocess 流式处理 3 个端点） | Done | P0 | EVO-103-B 子 Story；硬依赖 EVO-103-B-1（Done）；依赖 EVO-103-A（Done）；ITERATION-046 | [Item file](active/EVO-103-B-2-smart-http-endpoints.md)<br>[ADR-0004](../decisions/ADR-0004-git-centric-storage.md)<br>[EVO-103-B-1](active/EVO-103-B-1-git-client-auth-infra.md) |
| EVO-103-C | Repo Context API（file-tree/blobs/commits/diff） | Done | P0 | EVO-103 子 Story；硬依赖 EVO-103-A（ITERATION-048） | [Item file](active/EVO-103-C-repo-context-api.md)<br>[ADR-0004](../decisions/ADR-0004-git-centric-storage.md)<br>[EVO-103-A](active/EVO-103-A-repo-crud.md) |
