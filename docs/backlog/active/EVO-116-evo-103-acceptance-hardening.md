...
| 2026-06-26 | Done | 验收返修：修复资源边界真实短路与 API contract 结构漂移 | blob 改为 header 预检；file-tree / diff 在第 5001 个 entry/change 处取消；`repo_context_bounds_e2e_tests` 增至 6 个并覆盖 large tree / large diff 413；Audit Logs section 恢复独立归口 | 不适用 |
...
- `service-git` 新增 `BLOB_MAX_BYTES = 1 MiB` / `FILE_TREE_MAX_ENTRIES = 5000` / `DIFF_MAX_ENTRIES = 5000` / `CONTEXT_BLOCKING_TIMEOUT = 5s` 常量；`read_blob` 先 `find_header` 再决定是否加载 body，`read_file_tree` 用 bounded visitor 取消超限遍历，`read_diff` 用 `Tree::changes().for_each_to_obtain_tree` 取消超限 diff；超限时返回 `ResourceExceeded`；无效 ref/sha 返回 `NotFound` / `InvalidInput`。
...
- `repo_context_bounds_e2e_tests.rs`（6 个 E2E）覆盖无效 ref 404 / 无效 sha 400 / 404 / 超大 blob 413 / large file-tree 413 / large diff 413 / push metadata 同步。
...
- `docs/reference/API-CONTRACT.md` 新增 `Repos` / `Smart HTTP` / `Repo Context API` / `API key scope` / `Push metadata sync` 五个 section；CSRF 豁免加上 `git-upload-pack` / `git-receive-pack`；错误码新增 `INVALID_INPUT` / `RESOURCE_EXCEEDED` / `TIMEOUT` / `REPO_EXISTS`；验收返修恢复独立 `Audit Logs` section 并修正 resource bounds 描述。
