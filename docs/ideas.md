...
- [ ] **LLM 发现接口** | 每个工作台暴露统一的 LLM 接口, 返回该空间内所有可用能力的结构化描述, LLM 访问一个 URL 即可发现并理解全部资源 | 优先级：🔥 | 状态：💡
- [ ] **智能体自动接入引导** | 接口返回的内容包含接入指令, 引导本地智能体（如 OpenCode、Cursor 等）自动配置并接入工作台内的各项 MCP Server、Skill 和 Snippet | 优先级：🔥 | 状态：💡
