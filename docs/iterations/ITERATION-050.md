...
4. 本地 checkout 可用时：DOC-CHECK + git diff --check（recommended）
```
...
| 验证证据 | GitHub 文件读取、branch compare、PR #2 merge |
| 残余工作归口 | 运行时代码分别归 EVO-118-B~H；本地文档检查为推荐级补充验证 |
...
| 2026-07-30 | validation | `main...branch` compare 仅包含 `AGENTS.md` 与 `docs/**`；无运行代码/DB/脚本/部署配置变更 |
| 2026-07-30 | completion | PR #2 已 merge；EVO-118-A 更新为 Done，允许启动 EVO-118-B |
...
- 完成：治理 owner docs、P0/P1 Backlog、执行顺序和发布门禁已形成并合并到 `main`。
- 未完成：本环境无私有仓库本地 checkout，未执行推荐级 DOC-CHECK Python、Markdown link checker 和 `git diff --check`。
- 验证结果：GitHub Contents API 文件复核、Compare、PR 文件清单和 merge 状态均已确认；代码测试不适用。
- 闭环状态：`Complete`
- 残余归口：运行时修复按 EVO-118-B~H 逐项启动；推荐级本地文档检查不阻塞本治理 Story。

## 11. Retrospective

- 做得好的：将主观体检结论转换成发布 Gate、子 Story、失败模式和命令级验收，避免只留在对话中。
- 需要调整的：后续每轮必须验证 owner docs 与派生 Board 的状态，避免治理文档再次领先于真实代码。
- 写入 EVOLUTION：本轮未发现需要升级为新经验规则的额外陷阱。