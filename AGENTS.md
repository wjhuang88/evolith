...
影响超过一个文件的决策前，先分类已知约束：

| 类型 | 含义 | 处理方式 |
|------|------|----------|
| **Hard** | 不可变事实（平台限制、不可逆操作、外部合约） | 推导必要门禁 |
| **Soft** | 可协商的约定或偏好（代码风格、API 范式、工具选择） | 仅在选择影响其时记录到 `docs/decisions/` |
| **Assumption** | 未验证的假设（负载预期、兼容性声明） | 标记待验证；阻塞时创建 Spike |

区分 Hard 和 Soft 防止过度工程：只对 Hard 约束推导门禁，Soft 约束和 Assumption 不需要门禁。
当内心出现"标准做法是 X"或"我见过这个模式"时：(1) 说出你的假设；(2) 分类它；
(3) 如果是 Soft 或 Assumption，问自己：只满足 Hard 约束的最简方案是什么？
...
- 匹配既有风格，即使你不会那样写。
- 注意到无关的废弃代码时提一句——但不删它。
- 你自己的变更产生的孤儿（废弃的 import/变量/函数）要清理；先于你变更就已废弃的不动。
- 检验标准：每个变更行都能追溯到用户请求。
...
13. 不要从治理模板或其他项目推断本项目特性（数据库类型、包管理器、认证方案、发布
    模型等）；每个门禁必须追溯到本项目的 Hard 约束，而非通用模式。
14. Release 构建产物检查：生产构建不得包含 sourcemap、硬编码 secret 或调试环境变量注入。
...
MySQL is not supported by the main service. `DATABASE__DATABASE_TYPE=mysql` fails during config
validation / pool creation because MySQL repositories are not implemented. Use PostgreSQL for
production.
